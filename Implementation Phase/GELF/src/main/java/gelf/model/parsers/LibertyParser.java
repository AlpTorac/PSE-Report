...
/**
 * Parses the Liberty Files to their corresponding data objects so that they can
 * be easily manipulated and displayed in graphs
 * 
...
public class LibertyParser {
    ...
    // The format of an attribute's or parameter's name, not implemented because of
    // runtime
    ...
    /*
     * private static final String PARAMFORMAT = ATTRNAMEFORMAT + "\\:(\"\\(" +
     * FUNCTIONFORMAT + "\\)\"|\"" + NAMEFORMAT + "\"|" + FLOATFORMAT + "|" +
     * ATTRNAMEFORMAT + "|\"" + ATTRNAMEFORMAT + "\");";
     */

    // The format of an array, not implemented because of runtime
    /*
     * private static final String ARRAYFORMAT = "\"((" + FLOATFORMAT +",)(\\\\)?)*"
     * + FLOATFORMAT + "\"";
     */

    // The format of an array with a depth of 2, not implemented because of runtime
    /*
     * private static final String DOUBLEARRAYFORMAT = "(" + ARRAYFORMAT +
     * ",(\\\\)?)*" + ARRAYFORMAT;
     */

    // The format of an input Pin Attribute, not implemented because of runtime
    /*
     * private static final String INATTRIBUTESFORMAT = ATTRNAMEFORMAT + "\\(" +
     * NAMEFORMAT + "\\)\\{" + "index_1\\(" + ARRAYFORMAT + "\\);values\\(" +
     * ARRAYFORMAT + "\\);\\}";
     */

    // The format of an output Pin Attribute, not implemented because of runtime
    /*
     * private static final String OUTATTRIBUTESFORMAT = ATTRNAMEFORMAT + "\\(" +
     * NAMEFORMAT + "\\)\\{" + "index_1\\(" + ARRAYFORMAT + "\\);index_2\\(" +
     * ARRAYFORMAT + "\\);values\\(" + DOUBLEARRAYFORMAT + "\\);\\}";
     */

    // The format of a Leakage Attribute, not implemented because of runtime
    /*
     * private static final String LEAKAGEFORMAT = "leakage_power\\(\\)\\{when:\"" +
     * FUNCTIONFORMAT + "\";value:\"" + FLOATFORMAT + "\";\\}";
     */
    ...
    /*
     * private static final String PINFORMAT = "pin\\(" + NAMEFORMAT + "\\)\\{(" +
     * PARAMFORMAT + ")+((internal_power|timing)\\(" + NAMEFORMAT + "\\)\\{(" +
     * PARAMFORMAT + ")*((" + INATTRIBUTESFORMAT + ")+|(" + OUTATTRIBUTESFORMAT +
     * ")+)\\})+\\}";
     */

    // The format of a cell, not implemented because of runtime
    /*
     * private static final String CELLFORMAT = "cell\\(" + NAMEFORMAT + "\\)\\{(" +
     * PARAMFORMAT + ")+(" + LEAKAGEFORMAT + ")+(" + PINFORMAT + ")+\\}";
     */
     ...
     */
    /**
     * Recalculates the regex that separates power and timing groups, if the need to
     * change it dynamically arises in the future
     */
     ...
     * @throws InvalidFileFormatException if the format of the File is not
     *                                    recognised
     */
    public static Library parseLibrary(String libraryString) throws InvalidFileFormatException {
        ...
        if (libraryString == null || libraryString.length() == 0) {
            throw new InvalidFileFormatException("File is empty or file content" + " can't be read");
        }
        ...
        /*
         * if (!noCommentsString.matches(LIBRARYFORMAT)) { throw new
         * InvalidFileFormatException("Library doesn't fit format"); }
         */
        // Splits text by cell
        ...
        for (int i = 1; i < childCells.size(); i++) {
            ...
            if (!Arrays.equals(currentCell.getIndex1(), index1) || !Arrays.equals(currentCell.getIndex2(), index2)) {
     ...
     * @throws InvalidFileFormatException if the format of the Cell is not
     *                                    recognised
     */
    public static Cell parseCell(String cellString, String path) throws InvalidFileFormatException {
        ...
        if (java.lang.Math.pow(2, childInPins.size()) != leakages.getValues().length) {
            throw new InvalidFileFormatException("Invalid number of leakage entries " + "in cell \"" + path + "\"");
        }
        ...
        Cell productCell = new Cell(name, index1, index2, null, childInPins, childOutPins, leakages, defaultLeakage);
    ...
    /**
     * Parses a String data assuming it is a Pin. It checks midway if it is an input
     * Pin or an output Pin so that it can decide which format to expect. It is done
     * this way since when parsing new Files, we don't initially know which pin to
     * expect
     * 
     ...
     *                                    recognised
     */
    public static Pin parsePin(String pinString, ArrayList<InputPin> relatedPins, String path)
            throws InvalidFileFormatException {
        pinString = pinString.replaceAll("\\s+", "");
        // checks pin format, removed because of exceeding runtime
        /*
         * if (!pinString.matches(PINFORMAT)) { throw new
         * InvalidFileFormatException("Pin doesn't match format"); }
         */
        ...
        for (int i = 0; i < singleValueAttributes.length; i++) {
            ...
            case "direction":
                if (attrValue.equals("input")) {
                    ...
                    throw new InvalidFileFormatException(
                            "Direction \"" + attrValue + "\" in pin" + " \"" + path + "\" not supported");
                }
                break;
            case "function":
                if (!attrValue.matches("\"\\(" + FUNCTIONFORMAT + "\\)\"")) {
                    throw new InvalidFileFormatException(
                            "Function format: \"" + attrValue + "\" for Pin \"" + path + "\" is invalid");
                }
            ...
            case "max_capacitance":
                if (!attrValue.matches(FLOATFORMAT)) {
                    throw new InvalidFileFormatException(
                            "Value format for \"" + attrName + "\" in Pin \"" + path + "\" is invalid");
                }
                if (hasCapacitance) {
                    throw new InvalidFileFormatException(attrName + " in Pin \"" + path + "\" is defined twice");
                }
                hasCapacitance = true;
        ...
        if (isInput) {
            ...
            return parseOutputPin(attributes, name, hasCapacitance, hasMinCapacitance, maxCap, minCap, function,
                    unsupportedData);
        }
    }

    /**
     * Parses the attributes of an output Pin from an array of Strings with it's
     * attributes and the pins related to them
     * 
     ...
     * @throws InvalidFileFormatException if the stated related pins don't match the
     *                                    ones provided or the format of the
     *                                    attributes doesn't match
     */
    private static ArrayList<OutAttribute> parseOutAttributes(String[] attributeStrings,
        ...
        for (int i = 1; i < attributeStrings.length; i++) {
            ...
            if (attributeData.startsWith("internal_power")) {
                ...
                for (String attributeParam : powerAttributeParameters) {
                    ...
                    case "related_pin":
                        relatedPinString = powerAttributeParamParts[1].substring(1,
                                powerAttributeParamParts[1].length() - 1);
                ...
                for (int j = 1; j < powerAttributes.length; j++) {
                    ...
                    if (relatedPin == null && relatedPinString != null) {
                        throw new InvalidFileFormatException("Related Pin \"" + relatedPinString + "\"" + " on Pin \""
                                + path + "\" was not found before it");
                    } else if (relatedPin == null) {
                        throw new InvalidFileFormatException("Nonexistant related Pin on Pin \"" + path + "\"");
                    }
                ...
                // Parses timing for the output pin. Therefore ensures to fetch the relatedPin
                // first
                String relatedPinString = null;
                ...
                for (String attributeParam : timingAttributeParameters) {
                    ...
                    case "related_pin":
                        relatedPinString = value.substring(1, value.length() - 1);
                        if (!value.matches("\"" + NAMEFORMAT + "\"")) {
                            throw new InvalidFileFormatException(
                                    "Name format of related_pin " + value + " of Pin \"" + path + "\" is invalid");
                        }
    ...
    /**
     * Parses the attributes of an input Pin from an array of Strings with it's
     * attributes
     * 
     * @param attributeStrings the array with String representations of attributes
     * @param path             The path to these attributes
     * @return an array with the parsed attributes
     * @throws InvalidFileFormatException if the format of the attributes doesn't
     *                                    match
     */
    private static ArrayList<InAttribute> parseInAttributes(String[] attributeStrings, String path)
            throws InvalidFileFormatException {
        ...
        for (int i = 1; i < attributeStrings.length; i++) {
            ...
            if (attributeData.startsWith("internal_power")) {
                ...
                /*
                 * Timing for input pins not supported and therefore ignored, although it can be
                 * found in some Liberty files
                 */
            } else {
                /* no other known multiple value attributes. Can be implemented later */
            }
     ...
     * @param unsupportedData file data that hierarchically belong to a pin that the
     *                        program does not support
     * @return the initialised input Pin
     */
    private static InputPin parseInputPin(ArrayList<InAttribute> attributes, String name, boolean hasCapacitance,
            float capacitance, String unsupportedData) {
        /*
         * deprecated unless errors noticed // unifies attribute indexes / if
         * (attributes.size() != 0) { float[] uniformIndex1 = null; for (Attribute
         * attribute : attributes) { if (attribute instanceof InAttribute) { InAttribute
         * inAttribute = (InAttribute) attribute; float[] index1 =
         * inAttribute.getIndex1(); if (uniformIndex1 == null) { uniformIndex1 = index1;
         * } if (!Arrays.equals(index1, uniformIndex1)) { Interpolator interpolator =
         * new Interpolator(); float[] newValues = interpolator.interpolate(index1,
         * inAttribute.getValues(), uniformIndex1);
         * inAttribute.setIndex1(uniformIndex1); inAttribute.setValues(newValues); } } }
         * }
         */
     ...
     * @param attributes        the attributes of the inputPin
     * @param name              the name of the pin
     * @param hasMaxCapacitance whether the input pin has maximal capacitance
     *                          specified
     * @param hasMinCapacitance whether the input pin has minimal capacitance
     *                          specified
     ...
     * @param unsupportedData   file data that hierarchically belong to a pin that
     *                          the program does not support
     * @return
     */
    private static OutputPin parseOutputPin(ArrayList<OutAttribute> attributes, String name, boolean hasMaxCapacitance,
            boolean hasMinCapacitance, float maxCap, float minCap, String function, String unsupportedData) {
        /*
         * deprecated unless errors pop up in the future // unifies attribute indexes
         * float[] uniformIndex1 = null; float[] uniformIndex2 = null; for (Attribute
         * attribute : attributes) { if (attribute instanceof OutAttribute) {
         * OutAttribute outAttribute = (OutAttribute) attribute; float[] index1 =
         * outAttribute.getIndex1(); float[] index2 = outAttribute.getIndex2(); if
         * (uniformIndex1 == null | uniformIndex2 == null) { uniformIndex1 = index1;
         * uniformIndex2 = index2; } if (!Arrays.equals(uniformIndex1, index1) ||
         * !Arrays.equals(uniformIndex2, index2)) { Interpolator interpolator = new
         * Interpolator(); float[][] newValues = interpolator.bicubicInterpolate(index1,
         * index2, outAttribute.getValues(), uniformIndex1, uniformIndex2);
         * outAttribute.setIndex1(uniformIndex1); outAttribute.setIndex2(uniformIndex2);
    ...
    /**
     * Parses an array that contains Internal Power information on an output Pin to
     * give the OutputPower object
     * 
     ...
     * @throws InvalidFileFormatException if the Format of the Internal Power
     *                                    attribute doesn't fit expected format
     */
    ...
    /**
     * Parses an array that contains Internal Power information on an input Pin to
     * give the InputPower object
     * 
     * @param content the string of the Internal Power attribute
     * @param path    the path to the Internal Power
     * @return the parsed Internal Power Attribute
     ...
     * @throws InvalidFileFormatException if the Format of the Timing attribute
     *                                    doesn't fit expected format
     */
    ...
    /**
     * Parses the string of an attribute, but looking for an array of the parameter
     * arrayName, so that indexes can be parsed for attributes
     * 
    ...
    private static float[][] parseDoubleArray(String content, String arrayName, String path, String atrName)
        ...
        for (float[] array : result) {
            if (array.length != uniformLength) {
                throw new InvalidFileFormatException(
                        "Values have differen array lengths " + "in " + atrName + " of Pin \"" + path + "\"");
            }
        }
        return result;
    }
    ...
    /**
     * Checks the indexes on an output Pin of a Cell so that the Cell (and also its
     * parent Library) can get the indexes
     * 
     * @param outPins the output Pins of the cell
     * @return An array containing index1 array in it's first index and index2 in
     *         it's second
     */
    ...
    /**
     * @deprecated Because it server no vital function, only makes the code less
     *             flexible Checks whether the brackets are respected in the file,
     *             so that it can reject files with corrupted format
     * @param content The String to be checked
     * @throws InvalidFileFormatException if the String doesn't respect brackets
     */
    /*
     * private static void checkBrackets(String content) throws
     * InvalidFileFormatException { Stack<Integer> roundStack = new
     * Stack<Integer>(); Stack<Integer> curlyStack = new Stack<Integer>();
     * 
     * CharacterIterator it = new StringCharacterIterator(content);
     * 
     * while (it.current() != CharacterIterator.DONE) { switch (it.current()) { case
     * '(': roundStack.add(it.getIndex()); break; case ')': if
     * (roundStack.isEmpty()) { throw new
     * InvalidFileFormatException("Closed bracket in index " + it.getIndex() +
     * " although no bracket was opened"); } if (curlyStack.isEmpty()) {
     * roundStack.pop(); } else if (curlyStack.lastElement() > roundStack.pop()) {
     * throw new InvalidFileFormatException("Closed bracket in index " +
     * it.getIndex() + " although was supposed to close curly " + "brackets first");
     * } break; case '{': curlyStack.add(it.getIndex()); break; case '}': if
     * (curlyStack.isEmpty()) { throw new
     * InvalidFileFormatException("Closed bracket in index " + it.getIndex() +
     * " although no bracket was opened"); } if (roundStack.isEmpty()) {
     * curlyStack.pop(); } else if (curlyStack.pop() < roundStack.lastElement()) {
     * throw new InvalidFileFormatException("Closed bracket in index " +
     * it.getIndex() + " although was supposed to close round " + "brackets first");
     * } break; } it.next(); } if (!curlyStack.isEmpty()) { throw new
     * InvalidFileFormatException("Unclosed curly bracket from index " +
     * it.getIndex()); } else if (!roundStack.isEmpty()) { throw new
     * InvalidFileFormatException("Unclosed round bracket from index " +
     * it.getIndex()); } }
     */
}