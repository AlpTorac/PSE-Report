package gelf.view.composites;
import java.awt.Color;

public class ColorTheme {
    public static Color text = new Color(0xD3D3D3);

    public static Color frame = new Color(0x151515);
    public static Color section = new Color(0x242424);
    public static Color subsection = new Color(0x2E2E2E);
    public static Color subsubsection = new Color(0x424242);

    public static Color inputfield = new Color(0x242424);
    public static Color interactable = new Color(0x242424);

    public static Color liberty = new Color(0x60D479);
    public static Color hover = new Color(0x30613b);
    public static Color highlight = new Color(0x63980D);
    public static Color selected = new Color(0x357847);
    public static Color active = new Color(0x38c756);
}
